...
const endpoints = {
  ...
  own: (factId, userId) =>
    `/data/facts?where=_id%3D%22${factId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
  like: "/data/likes",
  totalLikes: (factId) =>
    `/data/likes?where=factId%3D%22${factId}%22&distinct=_ownerId&count`,
  userLikes: (factId, userId) =>
...
describe("E2E tests", function () {
  ...
  describe("Authentication [ 20 Points ]", function () {
    ...
    it("Login with valid input makes correct API call [ 2.5 Points ]", async function () {
      ...
      let passwordElement = await page.waitForSelector('[name="password"]', {
        timeout: interval,
      });
    ...
    it("Logout makes correct API call [ 2.5 Points ]", async function () {
      ...
      const [request] = await Promise.all([onRequest(), logoutBtn.click()]);
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    it("Logged user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      //Test for navigation
      await page.waitForSelector("nav >> text=Fun Facts", {
        timeout: interval,
      });
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(await page.isVisible("nav >> text=Add Fact"), "Create is visible")
        .to.be.false;
    ...
    it("Guest user navigation should work [ 2.5 Points ]", async function () {
      ...
      let funFactsBtn = await page.waitForSelector("nav >> text=Fun Facts", {
        timeout: interval,
      });
      ...
      let registerBtn = await page.waitForSelector("nav >> text=Register", {
        timeout: interval,
      });
      await registerBtn.click();
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#dashboard", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Add Fact", {
        timeout: interval,
      });
  ...
  describe("Home Page [ 5 Points ]", function () {
    it("Show Home page text [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(
        "text=Welcome to our website, where curiosity meets enjoyment!",
        { timeout: interval }
      );
      expect(
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    it("Show Fun facts page - welcome message [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("main >> text=Fun Facts", {
        timeout: interval,
      });
    ...
    it("Check Fun facts page with 0 Fun facts [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("text=No Fun Facts yet.", {
        timeout: interval,
      });
    ...
    it("Check fun facts have correct images [ 2.5 Points ]", async function () {
      ...
      const images = await page.$$eval(".fact img", (t) => t.map((s) => s.src));
    ...
    it("Check fun facts have more info button [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(".fact >> text=More Info", {
        timeout: interval,
      });
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addFactButton = await page.waitForSelector("text=Add Fact", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let categoryElement = await page.waitForSelector('[name="category"]', {
          timeout: interval,
        });
        let imageElement = await page.waitForSelector('[name="image-url"]', {
          timeout: interval,
        });
        ...
        const [request] = await Promise.all([onRequest(), submitBtn.click()]);
      });
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let moreInfoButton = await page.waitForSelector(
          `.fact:has-text("${data.description}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let categoryElement = await page.waitForSelector("#details-category", {
          timeout: interval,
        });
        let descriptionElement = await page.waitForSelector("#description", {
          timeout: interval,
        });
        ...
        expect(await page.isVisible('#action-buttons >> text="Edit"')).to.equal(
          false,
          "Edit button was visible for non-owner"
        );
      ...
      it("Details with owner shows correct info [ 2.5 Points ]", async function () {
        ...
        expect(await page.isVisible('#action-buttons >> text="Edit"')).to.equal(
          true,
          "Edit button was NOT visible for owner"
        );
    ...
    describe("Edit [ 17.5 Points ]", function () {
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let categoryInput = await page.waitForSelector('[name="category"]', {
          timeout: interval,
        });
        let imageInput = await page.waitForSelector('[name="image-url"]', {
          timeout: interval,
        });
      ...
      it("Edit sends correct headers [ 2.5 Points ]", async function () {
        ...
        let [request] = await Promise.all([onRequest(), submitBtn.click()]);
  ...
  describe("BONUS: Likes [ 15 Points ]", async () => {
    it("Likes for guests calls correct API and shows correct info [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible('#action-buttons >> text="Like"')).to.equal(
        false,
    ...
    it("Likes for owner calls correct API and shows correct info [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible('#action-buttons >> text="Like"')).to.equal(
        false,
        "Like button was visible for owner"
      );
      expect(totalLikesisHandled()).to.equal(
        true,
        "Total Likes API was not called"
    ...
    it("Liking for valid user sends correct API call [ 2.5 Points ]", async function () {
      ...
      let [request] = await Promise.all([likeOnRequest(), likeButton.click()]);
